        ...
        speechRecognizer.setRecognitionListener(new RecognitionListener() {
            ...
            editText.setText(data.get(0));
                ...
                if(data.get(0).contentEquals("show me a graph")) {
                    ...
                    //Glide.with(MainActivity.this).load(graphPictureLocation).centerCrop().placeholder(R.drawable.image_progress).into(graph);
                    ImageView simpleImageView=(ImageView) findViewById(R.id.image1);
                    simpleImageView.setImageResource(R.drawable.graph);
                }
...
//                        getJobDataById(int jobId)
                        ...
                        if (!graphPictureLocation.equals("")) {
                            // We now have the URL to the image on the server
                            Toast.makeText(MainActivity.this, "Picture Link Acquired", Toast.LENGTH_SHORT).show();
                        }

                    } else {
                        Toast.makeText(MainActivity.this, "Waiting on job creation task.", Toast.LENGTH_SHORT).show();
                    }
        ...
        call.enqueue(new Callback<List<GetMeasurementDataByIdPostResponse>>() {
            ...
            public void onResponse(Call<List<GetMeasurementDataByIdPostResponse>> call, Response<List<GetMeasurementDataByIdPostResponse>> response) {
                if(response.isSuccessful()) {
                    ...
                    Glide.with(MainActivity.this).load(graphPictureLocation).centerCrop().placeholder(R.drawable.image_progress).into(mImageView);
                }
                else
                {
                    Log.e("POST", response.toString());
